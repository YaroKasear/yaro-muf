...
: getConfig ( d s -- x )
    ...
    ( ref @ myProgKey @ "#" strcat propdir? if
        ref @ myProgKey @ "#" strcat array_get_proplist dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else ref @ myProgKey @ propdir? if
        ref @ myProgKey @ array_get_propvals dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else
        ref @ myProgKey @ getprop dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    ...
    ref @ myKey @ "#" strcat propdir? if
        ref @ myKey @ "#" strcat array_get_proplist dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else ref @ myKey @ propdir? if
        ref @ myKey @ array_get_propvals dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else
        ref @ myKey @ getprop dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    ...
    loc @ myProgKey @ "#" strcat propdir? if
        loc @ myProgKey @ "#" strcat array_get_proplist dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else loc @ myProgKey @ propdir? if
        loc @ myProgKey @ array_get_propvals dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else
        loc @ myProgKey @ getprop dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    ...
    loc @ myKey @ "#" strcat propdir? if
        loc @ myKey @ "#" strcat array_get_proplist dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else loc @ myKey @ propdir? if
        loc @ myKey @ array_get_proplist dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else
        loc @ myKey @ getprop dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    ...
    trigger @ ok? if
        trigger @ myKey @ "#" strcat propdir? if
            prog myKey @ "#" strcat array_get_proplist dup if
                dup cache_key @ swap cache_write exit
            else
                pop
            then
        else
            trigger @ myKey @ getprop dup if
                dup cache_key @ swap cache_write exit
            else
                pop
            then
        ...
        trigger @ myProgKey @ "#" strcat propdir? if
            trigger @ myProgKey @ array_get_proplist dup if
                dup cache_key @ swap cache_write exit
            else
                pop
            then
        else
            trigger @ myProgKey @ "#" strcat getprop dup if
                dup cache_key @ swap cache_write exit
            else
                pop
            then
    ...
    prog myProgKey @ "#" strcat propdir? if
        prog myProgKey @ "#" strcat array_get_proplist dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
    else
        prog myProgKey @ getprop dup if
            dup cache_key @ swap cache_write exit
        else
            pop
        then
